...
// Admin / Creator User ID (Aaditya Prakash)
...
async function seed() {
  console.log('Seeding 5 diverse quizzes with 25 total questions...');

  for (const qData of DIVERSE_QUIZZES) {
    ...
    if (quizError || !quiz) {
      console.error('Failed to create quiz:', qData.topic, quizError?.message);
      continue;
    }

    console.log(`Created Quiz [${quiz.id}]: ${quiz.topic}`);

    const questionsToInsert = qData.questions.map((q, idx) => ({
      quiz_id: quiz.id,
      prompt: q.prompt,
      options: q.options,
    ...
    const { error: qError } = await client.from('questions').insert(questionsToInsert);
    if (qError) {
      console.error(`Failed to insert questions for quiz ${quiz.id}:`, qError.message);
    } else {
